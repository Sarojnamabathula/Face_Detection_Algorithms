...
1. Clone this repository:
   ```bash
   git clone https://github.com/Sarojnamabathula/Face_Detection_Algorithms.git
   cd Face_Detection_Algorithms
   ```
2. Install the python dependencies listed in `face_detection/requirements.txt`:
   ```bash
...
You can run the face detection pipeline from the command line using `face_detection/main.py`.
...
```bash
python face_detection/main.py --image face_detection/images/Saroj.jpeg --algorithm haar
```
...
```bash
python face_detection/main.py --image face_detection/images/Saroj.jpeg --algorithm mtcnn
```
...
```bash
python face_detection/main.py --image face_detection/images/Saroj.jpeg --algorithm compare
```

### 4. Live Camera Comparison (with DroidCam / Webcam)
To compare algorithms in real-time side-by-side using a webcam or IP camera feed (like DroidCam):
```bash
python face_detection/main.py --camera --camera-src 0 --algorithm compare
```
Or stream directly from DroidCam via network URL:
```bash
python face_detection/main.py --camera --camera-src http://<phone-ip>:4747/video --algorithm compare
```
...
```bash
python face_detection/main.py --image face_detection/images/Saroj.jpeg --config custom_config.json
```
...
Create a file inside `face_detection/src/detectors/` (e.g. `face_detection/src/detectors/yolo_detector.py`).
...
```python
import cv2
from face_detection.src.detectors.base_detector import BaseDetector
from face_detection.src.detectors.detector_factory import DetectorFactory
...
Because of the dynamic registration, this new detector is automatically recognized. You can run it immediately:
```bash
python face_detection/main.py --image face_detection/images/Saroj.jpeg --algorithm yolo
```
